...
def get_gpu_info():
    ...
    try:
        result = subprocess.run(
            ['nvidia-smi', '--query-gpu=name,utilization.gpu,memory.used,memory.total,temperature.gpu,power.draw,clocks.current.graphics',
            ...
            for i, line in enumerate(result.stdout.strip().split('\n')):
                if line.strip():
                    ...
                    gpus.append({
                        ...
                        'utilization': float(parts[1]) if len(parts) > 1 and parts[1] != '[N/A]' else 0,
                        'mem_used': float(parts[2]) if len(parts) > 2 and parts[2] != '[N/A]' else 0,
                        'mem_total': float(parts[3]) if len(parts) > 3 and parts[3] != '[N/A]' else 0,
                        'temperature': float(parts[4]) if len(parts) > 4 and parts[4] != '[N/A]' else 0,
                        'power': float(parts[5]) if len(parts) > 5 and parts[5] not in ('[N/A]', 'N/A') else 0,
                        'clock': float(parts[6]) if len(parts) > 6 and parts[6] not in ('[N/A]', 'N/A') else 0,
                    })
            if gpus:
                return gpus
            ...
            for i, (key, val) in enumerate(data.items()):
                if key.startswith('card'):
                    gpus.append({
                        ...
                        'mem_total': 100,
                        'temperature': float(str(val.get('Temperature (Sensor edge) (C)', 0)).replace('c', '')),
                        'power': 0,
                        'clock': 0,
                    })
            if gpus:
...
def get_cpu_temps():
    try:
        ...
        for key in ('coretemp', 'k10temp', 'cpu_thermal', 'acpitz'):
            if key in temps:
                return [{'label': t.label or f'Core {i}', 'current': t.current, 'high': t.high, 'critical': t.critical}
                        for i, t in enumerate(temps[key])]
        # Fallback: first available
        first = next(iter(temps.values()))
        return [{'label': t.label or f'Sensor {i}', 'current': t.current, 'high': t.high, 'critical': t.critical}
...
def get_stats():
    ...
    disk_read_speed = (disk_io.read_bytes - _prev_disk.read_bytes) / disk_dt if disk_dt > 0 else 0
    disk_write_speed = (disk_io.write_bytes - _prev_disk.write_bytes) / disk_dt if disk_dt > 0 else 0
    _prev_disk = disk_io
    _prev_disk_time = now
    ...
    net_sent_speed = (net_io.bytes_sent - _prev_net.bytes_sent) / net_dt if net_dt > 0 else 0
    net_recv_speed = (net_io.bytes_recv - _prev_net.bytes_recv) / net_dt if net_dt > 0 else 0
    _prev_net = net_io
    _prev_net_time = now
...
def get_processes(sort_by='cpu', limit=50):
    ...
    attrs = ['pid', 'name', 'username', 'status', 'cpu_percent', 'memory_percent',
             'memory_info', 'num_threads', 'create_time', 'nice', 'cmdline']
